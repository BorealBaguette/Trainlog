...
def convert_motis_to_trip(itinerary, username):
    ...
    for leg in itinerary["legs"]:
        # Add start point
        path_coordinates.append({"lat": leg["from"]["lat"], "lng": leg["from"]["lon"]})
        ...
        if "legGeometry" in leg and leg["legGeometry"].get("points"):
            try:
                ...
                decoded_points = polyline.decode(
                    leg["legGeometry"]["points"], precision=precision
                )

                # Convert to our coordinate format, skip first/last to avoid duplicates
                for point in decoded_points[1:-1]:
                    path_coordinates.append({"lat": point[0], "lng": point[1]})

                print(
                    f"Decoded {len(decoded_points)} points with precision {precision} for {leg['mode']} leg"
                )
    ...
    path_coordinates.append(
        {"lat": last_leg["to"]["lat"], "lng": last_leg["to"]["lon"]}
    )
    ...
    return {"trip": trip_data, "path": path_coordinates}
...
def call_motis_api(forwardRouting=None):
    ...
    try:
            ...
            try:
                ...
                if "T" in time_param:
                    ...
                    if clean_time and len(clean_time) == 10:  # YYYY-MM-DD format
                        params["time"] = (
                            datetime.fromisoformat(f"{clean_time}T12:00:00")
                            .replace(tzinfo=tz)
                            .isoformat()
                        )
        ...
        if transit_modes_param and transit_modes_param.lower() not in ["none", ""]:
            transit_modes = [
                mode.strip().upper() for mode in transit_modes_param.split(",")
            ]
            valid_modes = [
                mode for mode in transit_modes if mode in valid_transit_modes
            ]
            if valid_modes:
        ...
        if direct_modes_param and direct_modes_param.lower() not in ["none", ""]:
            direct_modes = [
                mode.strip().upper() for mode in direct_modes_param.split(",")
            ]
        ...
        response = requests.get(
            "https://api.transitous.org/api/v3/plan", params=params, timeout=30
        )
...
def handle_search_form(username):
    ...
    try:
        ...
        if transit_modes:
            clean_transit_modes = [
                mode for mode in transit_modes if mode and mode != "None"
            ]
        ...
        if direct_modes:
            clean_direct_modes = [
                mode for mode in direct_modes if mode and mode != "None"
            ]
