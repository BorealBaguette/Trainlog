-- PostgreSQL. Multi-trip variant of get_trip.sql (same columns, WHERE ANY(:ids)) so
-- callers handling many trips at once pay one round-trip instead of one per trip.
-- Keep the SELECT list in sync with get_trip.sql. Row order is unspecified; callers
-- reorder by trip_id. Rows adapt back to the legacy SQLite `trip` shape in Python
-- (adapt_pg_trip_row); username is resolved from user_id by the caller.
WITH base AS (
    SELECT trips.*,
