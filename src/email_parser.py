...
from src.ai import (
    create_trip_from_parsed,
    extract_pdf_text,
    parse_ics_content,
    parse_trip_with_ai,
)
from src.users import User
from src.utils import lang, sendEmail
...
def get_user_from_sender(sender_raw):
    ...
    if not user.premium:
        ...
        language = lang.get(user.lang, lang["en"])
        try:
            sendEmail(
                email_address,
                language["email_premium_subject"],
                f"""<h2>{language["email_premium_title"]}</h2><p>{language["email_premium_greeting"].format(username=user.username)}</p><p>{language["email_premium_description"]}</p><p><a href="https://buymeacoffee.com/trainlog/membership">{language["email_premium_cta"]}</a></p>""",
            )
        except Exception as e:
...
def send_confirmation_email(user, created_trips, subject):
    ...
    trip_lines = [
        f"• {t.origin_station} → {t.destination_station} ({t.start_datetime.strftime('%Y-%m-%d') if t.start_datetime else '?'})"
        for t in created_trips
    ]
    language = lang.get(user.lang, lang["en"])
    sendEmail(
        user.email,
        language["email_success_subject"],
        f"""<h2>{language["email_success_title"]}</h2><p>{language["email_received"]}: <strong>{subject}</strong></p><p><strong>{len(created_trips)} {language["email_trips_added"]}</strong></p><p>{"<br>".join(trip_lines)}</p><p><a href="https://trainlog.me/public/trip/{trip_ids}">{language["email_view_trips"]}</a></p>""",
    )
...
def send_error_email(user, subject, error_message):
    ...
    try:
        sendEmail(
            user.email,
            language["email_error_subject"],
            f"""<h2>{language["email_error_title"]}</h2><p>{language["email_received"]}: <strong>{subject}</strong></p><p>{language["email_error_description"]}</p><p><em>{error_message}</em></p><p>{language["email_error_advice"]}</p>""",
        )
    except Exception as e:
...
def send_no_trips_email(user, subject):
    ...
    try:
        sendEmail(
            user.email,
            language["email_no_trips_subject"],
            f"""<h2>{language["email_no_trips_title"]}</h2><p>{language["email_received"]}: <strong>{subject}</strong></p><p>{language["email_no_trips_description"]}</p><p>{language["email_no_trips_formats"]}</p>""",
        )
    except Exception as e:
...
def process_incoming_email(raw):
    ...
    with _app.app_context():
        ...
        try:
            ...
            subject = (
                subject_raw.decode(enc or "utf-8")
        ...
        logger.info(
            f"Processing email from {user.username} (ICS: {len(ics_events)}, PDFs: {len(pdf_texts)})"
        )
        ...
        for i, parsed in enumerate(trips):
            try:
                trip = create_trip_from_parsed(
                    user, parsed, purchase_date, source="email"
                )
                if trip:
        ...
        if created_trips:
            ...
            if errors:
                send_error_email(
                    user,
                    subject,
                    f"Created {len(created_trips)} trip(s), but some failed: "
                    + "; ".join(errors),
                )
        else:
            send_error_email(
                user,
                subject,
                "Could not create any trips. " + "; ".join(errors)
                if errors
                else "Unknown error.",
            )
