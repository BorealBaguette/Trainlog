...
def get_wrapped_data(username, year, trip_type="combined"):
    ...
    with pg_session() as pg:
        ...
        if prev_totals and prev_totals.total_trips > 0:
            ...
            wrapped["trips_change"] = round(
                (
                    (wrapped["total_trips"] - wrapped["prev_trips"])
                    / wrapped["prev_trips"]
                )
                * 100
            )
            wrapped["km_change"] = (
                round(
                    ((wrapped["total_km"] - wrapped["prev_km"]) / wrapped["prev_km"])
                    * 100
                )
                if wrapped["prev_km"] > 0
                else None
            )
        else:
        ...
        if time_results:
            ...
            wrapped["time_of_day"] = {
                ...
                "favorite": max(time_breakdown, key=time_breakdown.get)
                if time_breakdown
                else None,
                "favorite_percent": round(
                    (max(time_breakdown.values()) / total_time_trips) * 100
                )
                if total_time_trips > 0
                else 0,
            }
        else:
        ...
        wrapped["unique_stations"] = (
            int(stations_result.unique_stations) if stations_result else 0
        )
        ...
        if percentile_result and percentile_result.total_users > 1:
            ...
            wrapped["percentile"] = {
                "km": round(km_percentile, 2)
                if km_percentile > 98
                else int(round(km_percentile)),
                "trips": round(trips_percentile, 2)
                if trips_percentile > 98
                else int(round(trips_percentile)),
    ...
    wrapped["top_routes"] = []
    for r in routes[:3]:
        route_value = r.get("route")

        if isinstance(route_value, str):
            try:
                parsed = json.loads(route_value)
        ...
        if isinstance(parsed, list) and all(isinstance(x, str) for x in parsed):
            route_str = " → ".join(parsed)
        else:
            route_str = route_value or "Unknown"

        wrapped["top_routes"].append(
            {"name": route_str, "count": int(r.get("count", 0))}
        )
    ...
    if wrapped["days_traveled"] > 0:
        wrapped["year_percent"] = round(
            (wrapped["total_duration"] / (365 * 86400)) * 100, 1
        )
    else:
