...
def suggest_cuts(path: Path) -> list[int]:
    ...
    result is only a suggestion; the UI lets the owner add, move and delete cuts.
    """
...
def _segments(cuts: list[int], width: int) -> list[tuple[int, int]]:
    """Turn cut positions into (x0, x1) spans covering the whole strip."""
    bounds = [0, *sorted(set(cuts)), width]
    return [(bounds[i], bounds[i + 1]) for i in range(len(bounds) - 1)]


def _distinct_sides(left: Path, right: Path) -> bool:
    """
    True if `right` is a real second drawing rather than a duplicate of `left`.

    VagonWeb only ships a "-b" image for stock whose two sides actually differ; that
    image is already drawn in the reversed orientation, so car *i* of an L strip sits
    at the mirrored x-range in it. When the two files are identical there is really
    only one drawing and the R side has to be generated by flipping.
    """
...
def _crop_side(src: Path, dest: Path, x0: int, x1: int, *, mirror_box: bool = False) -> None:
    """
    Write the [x0, x1) slice of `src` to `dest`.

    `mirror_box` takes the slice from the mirrored x-range, for reading car *i* out of
    the "-b" strip: it depicts the same unit from the other side, so the car that is
    leftmost in "-a" is rightmost there.
    """
...
def preview():
    ...
    for w in parsed["wagons"]:
        ...
        if ext_r and not _distinct_sides(path_l, STAGING_DIR / f"{key}_R.{ext_r}"):
...
def import_one():
    ...
    two_sided = bool(src_r) and _distinct_sides(src_l, src_r)
