...
from src.utils import get_default_trip_visibility, managed_cursor, pathConn
    ...
    try:
        with pg_session() as pg:
            result = pg.execute(
                ...
                LIMIT 1
            """,
                {"name": station_name, "trip_type": trip_type},
            ).fetchone()
        ...
        with managed_cursor(pathConn) as cursor:
            cursor.execute(
                "SELECT path FROM paths WHERE trip_id = ?", (result.trip_id,)
            )
            row = cursor.fetchone()
...
def geocode_station(query, trip_type="train", fallback_coords=None, city_fallback=None):
    ...
    for q in queries_to_try:
        ...
        for url in [
            "http://photon.srv.trainlog.me",
            "https://photon.chiel.uk/api",
...
def parse_trip_with_ai(
    text, user_lang="en", images=None, ics_events=None, pdf_texts=None
):
    config = load_config()
...
Text: {text if text else "(see images)"}{attachment_info}"""
    ...
    try:
        ...
        valid_trips = [
            t
            for t in parsed
            if isinstance(t, dict) and t.get("origin") and t.get("destination")
        ]
...
def enrich_parsed_trip(parsed_trip):
    ...
    if trip_type == "air":
        ...
        parsed_trip["_resolved_origin"] = (
            f"{origin_flag} {origin_airport['name']} ({origin_iata.upper()})"
        )
        parsed_trip["_resolved_destination"] = (
            f"{dest_flag} {dest_airport['name']} ({dest_iata.upper()})"
        )
        parsed_trip["_origin_coords"] = {
            "lat": origin_airport["latitude"],
            "lng": origin_airport["longitude"],
        }
        parsed_trip["_dest_coords"] = {
            "lat": dest_airport["latitude"],
            "lng": dest_airport["longitude"],
        }
        parsed_trip["_path"] = [
            parsed_trip["_origin_coords"],
            parsed_trip["_dest_coords"],
        ...
        origin_fallback = (
            (parsed_trip["origin_lat"], parsed_trip["origin_lng"])
            if parsed_trip.get("origin_lat") and parsed_trip.get("origin_lng")
            else None
        )
        dest_fallback = (
            (parsed_trip["destination_lat"], parsed_trip["destination_lng"])
            if parsed_trip.get("destination_lat") and parsed_trip.get("destination_lng")
            else None
        )

        origin_geo = geocode_station(
            parsed_trip.get("origin", ""), trip_type, origin_fallback
        )
        dest_geo = geocode_station(
            parsed_trip.get("destination", ""), trip_type, dest_fallback
        )

        if not origin_geo or not dest_geo:
            logger.warning(
                f"Could not geocode: {parsed_trip.get('origin')}, {parsed_trip.get('destination')}"
            )
            return None
        ...
        routed_path = route_path(
            parsed_trip["_origin_coords"], parsed_trip["_dest_coords"], trip_type
        )
        parsed_trip["_path"] = (
            routed_path
            if routed_path
            else [parsed_trip["_origin_coords"], parsed_trip["_dest_coords"]]
        )

    parsed_trip["_distance"] = getDistance(
        parsed_trip["_path"][0], parsed_trip["_path"][-1]
    )
    return parsed_trip
...
def create_trip_from_parsed(user, parsed_trip, purchase_date=None, source="ai"):
    ...
    if (
        "_resolved_origin" not in parsed_trip
        or "_resolved_destination" not in parsed_trip
    ):
    ...
    if trip_type == "air":
        ...
        if not origin_station or not dest_station or not origin_point or not dest_point:
            ...
            dest_fallback = (
                (parsed_trip["destination_lat"], parsed_trip["destination_lng"])
                if parsed_trip.get("destination_lat")
                and parsed_trip.get("destination_lng")
                else None
            )
    ...
    if utc_start:
        utc_start_datetime = (
            utc_start.replace(tzinfo=None)
            if hasattr(utc_start, "replace")
            else utc_start
        )
        start_datetime = getLocalDatetime(
            path[0]["lat"],
            path[0]["lng"],
            utc_start.replace(tzinfo=pytz.UTC)
            if utc_start.tzinfo is None
            else utc_start,
        )
        if utc_end:
            utc_end_datetime = (
                utc_end.replace(tzinfo=None) if hasattr(utc_end, "replace") else utc_end
            )
            end_datetime = getLocalDatetime(
                path[-1]["lat"],
                path[-1]["lng"],
                utc_end.replace(tzinfo=pytz.UTC) if utc_end.tzinfo is None else utc_end,
            )
        else:
    ...
    trip = Trip(
        ...
        reg=None,
        waypoints=None,
        notes=build_notes(parsed_trip, user.lang, source),
